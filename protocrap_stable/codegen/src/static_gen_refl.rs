...
pub fn generate_static_dynamic(value: &DynamicMessage) -> Result<TokenStream> {
    ...
    // Parse type path
    let path_parts: Vec<_> = value.descriptor()
        .name()
        .split(".")
    ...
    Ok(quote! {
        {
            #(#path_parts)::* ::ProtoType::from_static(
                #has_bits_tokens,
                #(#field_inits),*
            )
        }
...
fn calculate_has_bits(value: &DynamicMessage) -> Vec<u32> {
    ...
    for &field in descriptor.field() {
        ...
        if let Some(_) = value.get_field_optional(field) {
    ...
    for i in 0..word_count {
        // assert_eq!(has_bits[i], *value.object.ref_at::<u32>(i * 4), "has bits should match");
    }
...
fn generate_field_initializers(
    ...
    for field in fields {
        let value = value.get_field_optional(field);
...
fn generate_nested_message(msg: &DynamicMessage) -> Result<TokenStream> {
    ...
    Ok(quote! {
        {
            static PROTO_TYPE: #(#path_parts)::* ::ProtoType = #nested_initializer;
