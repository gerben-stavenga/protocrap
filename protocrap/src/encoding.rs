...
use crate::{
    Protobuf, base::Object, containers::Bytes, tables::{AuxTableEntry, Table}, utils::{Stack, StackWithStorage}, wire::{FieldKind, SLOP_SIZE, WriteCursor, zigzag_encode}
};
...
fn encode_loop<'a>(
    ...
    'out: loop {
        ...
        match kind {
            ...
            FieldKind::Message => {
                let AuxTableEntry { offset, child_table } = Table::table(obj_state.table).aux_entry(offset);
                let child_ptr = obj_state.get::<*const Object>(offset as usize);
                if !child_ptr.is_null() {
                    obj_state.field_idx -= 1;               
                    obj_state.push(tag, count(cursor, begin, byte_count), stack)?;
                    obj_state = ObjectEncodeState::new(unsafe { &*child_ptr }, unsafe { &*child_table });
