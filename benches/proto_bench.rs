...
fn bench_parse(c: &mut Criterion) {
    ...
    {
        let mut msg = Test::default();
        msg.parse_flat::<32>(black_box(medium_data.as_slice()));
        msg.parse_flat::<32>(black_box(medium_data.as_slice()));
    }
    println!("Prepared medium message for protocrap parsing benchmark");
    ...
    group.bench_function("large/protocrap", |b: &mut criterion::Bencher<'_>| {
        let mut msg = Test::default();
        b.iter(|| {
            msg.parse_flat::<32>(black_box(large_data.as_slice()));
            black_box(&msg as *const _);
        })
    });
